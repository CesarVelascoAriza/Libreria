package com.cava.examples.security.Seguridad.config;
...
public class ServerAutirizationConfigBasic {
    ...
    public RegisteredClientRepository registeredClientRepository() {
        RegisteredClient oidcClient = RegisteredClient.withId(UUID.randomUUID().toString())
                ...
                .redirectUri("http://localhost:8762/api/security/login/oauth2/code/oidc-client")
                .redirectUri("http://localhost:8762/api/security/authorized")
                .postLogoutRedirectUri("http://localhost:8762/")
    ...
    public OAuth2TokenCustomizer<JwtEncodingContext> tokenCustomizer() {
        return context -> {
            if (context.getTokenType().getValue().equals(OAuth2TokenType.ACCESS_TOKEN.getValue())) {
                ...
                .claim("roles", auth.getAuthorities().stream().map(
